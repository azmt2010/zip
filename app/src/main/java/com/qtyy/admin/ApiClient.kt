...
object ApiClient {
    const val BASE_URL = "http://45.116.78.241"
    ...
    private fun connection(path: String, method: String, auth: Boolean): HttpURLConnection {
        ...
        return (url.openConnection() as HttpURLConnection).apply {
            ...
            setRequestProperty("User-Agent", "QTYAdmin-Android/1.0.1-http")
    ...
    suspend fun dashboard(): Dashboard {
        ...
        return Dashboard(
            j.optInt("user_count"),
            j.optInt("online_count"),
            j.optInt("today_registration"),
            j.optInt("last_week_registration"),
            j.optInt("month_registration"),
        )
    }
    ...
    suspend fun userByCard(cardKey: String): UserDetailResult {
        ...
        return if (j.optBoolean("bound")) {
            UserDetailResult(true, userFrom(j.getJSONObject("user"), true), null)
        } else {
            UserDetailResult(false, null, cardFrom(j.getJSONObject("card")))
        }
    }
    ...
    ): UploadItem = withContext(Dispatchers.IO) {
    ...
    private fun userFrom(j: JSONObject, revealPassword: Boolean = false) = UserItem(
        ...
        if (revealPassword) nullableString(j, "password") else null,
        nullableString(j, "card_key"),
        nullableString(j, "hwid"),
        j.optBoolean("hwid_banned"),
        nullableString(j, "last_ip"),
        j.optBoolean("enabled", true),
        j.optString("created_at"),
        nullableString(j, "last_seen"),
    )

    private fun cardFrom(j: JSONObject) = CardItem(
        j.optInt("id"),
        j.optString("card_key"),
        j.optString("note"),
        j.optString("status"),
        j.optString("created_at"),
        nullableString(j, "used_at"),
        if (j.has("user_id") && !j.isNull("user_id")) j.optInt("user_id") else null,
        j.optBoolean("permanent", true),
    )

    private fun uploadFrom(j: JSONObject) = UploadItem(
        j.optInt("id"),
        j.optString("original_name"),
        j.optLong("size_bytes"),
        j.optString("sha256"),
        j.optString("note"),
        j.optString("uploaded_at"),
    )
}
...
data class Dashboard(
    ...
    val todayRegistration: Int = 0,
    val lastWeekRegistration: Int = 0,
    val monthRegistration: Int = 0,
)
